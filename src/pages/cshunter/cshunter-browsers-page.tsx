        ...
        <>
            {
                browsers.length > 0 ?
                    (<Flex height="full" justify="space-between">
                        <Tabs.Root height="full" paddingRight="40px" variant="plain" value={selectedBrowser ? selectedBrowser.id : ""} onValueChange={(e) => setSelectedBrowser(e.value ? browsers.find((browser) => browser.id == e.value) : undefined)} defaultValue={browsers.find((browser) => browser.support)?.id}>
                            <Flex spaceX={5}>
                                <Tabs.List>
                                    {
                                ...
                                <Input value={currentFilter} onChange={(e) => setCurrentFilter(e.target.value)} placeholder="Фильтр" borderRadius="20px" />
                            </Flex>

                            <Flex paddingTop={3} paddingBottom={5}>
                                <SelectRoot value={[selectedDataType]} onValueChange={(v) => setSelectedDataType(v.value[0])} positioning={{ placement: "bottom", flip: false }} collection={dataTypes}>
                                    ...
                                    <SelectContent borderWidth="1px" bg="#18181B" borderRadius={20}>
                                        {
                        ...
                        <Flex gap="5" height="fit" direction="column">
