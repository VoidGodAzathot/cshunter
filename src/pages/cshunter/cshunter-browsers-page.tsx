...
export default function CSHunterBrowsersPage() {
  ...
  const [browsersData, setBrowsersData] = useState<BrowserData | undefined>(
    undefined
  );
  const dataTypes = createListCollection({
    items: [
      { label: "Кэш", value: "cache" },
      { label: "Посещения", value: "visits" },
      { label: "Загрузки", value: "downloads" },
    ],
  });

  async function applyFilter() {
    const response = browsersData
      ? selectedDataType === "cache"
        ? await asyncFilter(
            browsersData.cache,
            async (item) =>
              item.browser == selectedBrowser?.id &&
              (await filterIsPresent(currentFilter, item))
          )
        : selectedDataType === "downloads"
        ? await asyncFilter(
            browsersData.downloads,
            async (item) =>
              item.browser == selectedBrowser?.id &&
              (await filterIsPresent(currentFilter, item))
          )
        ...
        <Flex height="full" direction="column" justify="space-between">
          <Flex spaceX={5}>
            ...
            <SelectRoot
              ...
              onValueChange={(v) => setSelectedDataType(v.value[0])}
            ...
            >
              ...
              >
                {dataTypes.items.map((item) => (
                  <SelectItem borderRadius={20} item={item} key={item.value}>
                    {item.label}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <Flex height="full" width="full">
            <Tabs.Root
              ...
              paddingRight="40px"
              variant="plain"
              value={selectedBrowser ? selectedBrowser.id : ""}
              onValueChange={(e) =>
                setSelectedBrowser(
                  e.value
                    ? browsers.find((browser) => browser.id == e.value)
                    : undefined
                )
              }
              defaultValue={browsers.find((browser) => browser.support)?.id}
            >
              <Flex spaceX={5} paddingY={5}>
                <Tabs.List>
                  {browsers.map((browser) => (
                    <Tabs.Trigger
                      disabled={!browser.support}
                      borderWidth={
                        selectedBrowser && selectedBrowser.id == browser.id
                          ? "1px"
                          : "0px"
                      }
                      borderRadius={20}
              ...
              {browsers.map((browser, i) => (
                <Tabs.Content
                  ...
                  value={browser.id}
                >
                  {BrowserTableDataWrapper(currentFilter, currentData)}
                </Tabs.Content>
              ))}
            </Tabs.Root>
            ...
            <Flex gap="5" paddingTop={5} height="fit" direction="column">
              <Stat.Root borderWidth="1px" p="4" rounded={20}>
                <Stat.Label fontSize={14}>Всего посещений</Stat.Label>
                <HStack>
              ...
              <Stat.Root borderWidth="1px" p="4" rounded={20}>
                <Stat.Label fontSize={14}>Всего загрузок</Stat.Label>
                <Stat.ValueText fontSize={18}>
              ...
              <Stat.Root borderWidth="1px" p="4" rounded={20}>
                <Stat.Label fontSize={14}>Размер кэша</Stat.Label>
                <Stat.ValueText fontSize={18}>
