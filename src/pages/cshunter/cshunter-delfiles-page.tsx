...
export default function CSHunterDelFilesPage() {
  ...
  const [paginatedData, setPaginatedData] = useState<FileRecord[]>([]);
  const [totalPages, setTotalPages] = useState<number>(0);

  useEffect(() => {
    setTotalPages(Math.ceil(currentFiles.length / pageSize));

    setPaginatedData(
      currentFiles.slice((currentPage - 1) * pageSize, currentPage * pageSize)
    );
  }, [currentFiles]);
  ...
  useEffect(() => {
    async function applyFilter() {
      const filtered = await asyncFilter(
        files,
        async (file) => await filterIsPresent(currentFilter, file)
      );
