...
export default function CSHunterAnalyzerPage() {
  ...
  const [sysContext, setSysContext] = useState<AnalyzeContext | undefined>(
    undefined
  );
  const [currentContext, setCurrentContext] = useState<
    AnalyzeContext | undefined
  >(undefined);
    ...
    async function applyFilter() {
      ...
      if (currentContext && sysContext) {
        ...
        for (let i = 0; i < currentContext.items.length; i++) {
          ...
          const fromSys = sysContext.items.filter(
            (item_2) => item.crc32 == item_2.crc32
          );
          const mapped = fromSys.map((item) => {
        ...
        <Flex height="full" justify="space-between" direction="column" gap={5}>
          <Flex gap={5} width="full">
              ...
              >
                {currentContext ? (
                  <Stat.Root
                    ...
                    gap={0}
                  >
                    <Stat.Label fontSize={12}>Всего файлов</Stat.Label>
                ...
                onClick={async () => {
                  try {
                    ...
                    if (clipboardBuf.startsWith("http")) {
                      const context: AnalyzeContext | undefined = await invoke(
                      ...
                      if (context) {
                        ...
                        toaster.create({
                          ...
                          description:
                            "Загрузка выполнена с ссылки из буффера обмена.",
                          type: "success",
                        });
                        return;
                      }
            ...
            >
              ...
              <Flex gap={2}>
                {sysContext ? (
                  sysContext.items.length > 0 ? (
                    <Badge borderRadius="20px" width="fit" colorPalette="green">
                      активен
                    </Badge>
                  ) : (
                    <Badge width="fit" borderRadius="20px" colorPalette="red">
                      не активен
                    </Badge>
                  )
                ) : (
                  <Badge borderRadius="20px" width="fit" colorPalette="red">
                    ошибка получения
                  </Badge>
                )}
                {sysContext ? (
                  <Badge borderRadius="20px" colorPalette="blue">
                    только .dll и .exe
                  </Badge>
                ) : (
                  <></>
              ...
              <Flex height="full" marginTop="auto" alignItems="end">
                <Button
                  onClick={async () => {
                    ...
                    if (dir) {
                      const files: string[] = await invoke(
                        "get_parallel_files",
                        { startPath: dir }
                      );
                      const context: AnalyzeContext | undefined = await invoke(
                      ...
                      if (context) {
                        ...
                        toaster.create({
                          ...
                          description:
                            'Файл сохранен в той же папке с названием "context.json".',
                          type: "success",
                        });
                        return;
                      }
          ...
          >
            ...
            >
              {isMatching || currentMatches.length == 0 ? (
                <Center gap={5} className="flex flex-col" height="full">
                  <Spinner size="xl" />
                  {isMatching ? (
                    <Text color="gray">Применение контекста</Text>
                  ) : (
                    <Text color="gray">Ожидание</Text>
                  )}
                </Center>
              ) : (
                <>
